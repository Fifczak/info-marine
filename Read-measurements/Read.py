...
#RUN FUNKCJA
def read_measurement_file(device):
	def get_meas():
		def readomnitrend():
			...
			def get_measdomain(measno):
				...
				if parent == 103:
					if measno == '1004' or measno == '1007' or measno == '1025' or measno == '1059': measdomain.append('FFT'), measdomain.append('Vel'), measdomain.append('[mm/s]')
					if measno == '1018' or measno == '1028' or measno == '1029': measdomain.append('FFT'), measdomain.append('Env'), measdomain.append('[m/s2]')

					else: measdomain.append('UNKNOWN'),measdomain.append('UNKNOWN'),measdomain.append('UNKNOWN')
					
				if parent == 46:
					if measno == '1004' or measno == '1007' or measno == '1026' or measno == '1028': measdomain.append('FFT'), measdomain.append('Vel'), measdomain.append('[mm/s]')
					if measno == '1023' or measno == '1025' or measno == '1027': measdomain.append('FFT'), measdomain.append('Env'), measdomain.append('[m/s2]')
		...
		def readezthomas():
			...
			for column in range(worksheet.ncols): # Tu szukamy pomiarów
				...
				if worksheet.cell(0,column).value == 'eZ-TOMAS Remote 8,0,40' or worksheet.cell(0,column).value == 'eZ-TOMAS 8,0,40': #Znalezienie raport sheeta
						...
						if worksheet.cell(row,column).value == '1.   Frequency (Hz)':
							...
							for row2 in range(measnumbers): # Szukanie nazw
								...
								for chartiter in range(worksheet.nrows):
									if worksheet.cell(measstart+chartiter,column+row2).value == xlrd.empty_cell.value: break
									x.chart.append(round(float(worksheet.cell(measstart+chartiter,column+row2).value),3))
									x.end = worksheet.cell(measstart+chartiter,column).value
									chartlist.append(round(float(worksheet.cell(measstart+chartiter,column+row2+1).value),3))
								x.chart = chartlist
								measlist.append(x)
								measlist.append(countoverall(x))
							break
	...
	def changepointWindow(point,id_):
		def change_pointname():
			...
			if not trypoint:
				querry = "UPDATE points set point = '" + str(changepointEntry.get()) + "' where point = '" + point + "' and id = " + str(id_)
				q_run(connD, querry)
	...
	def deviceinstructWindow(id_):
		...
		okbutton = tk.Button(PointsWindow, text = 'Change routename', command= lambda: change_routename(nameindevice.get()))
	...
	def window_crosstable():
		...
		def upload():
			...
			for i in measlist:
				...
				if i.mode == 'FFT':
					...
					querry = "INSERT INTO meascharts(shipid,id,point,report_number, date, domain, type, unit, chart) VALUES (" + str(parent) + "," + str(i.id) + ",'"	+ str(i.point) + "','" + str(rnumber)+ "','" +str(i.date)+ "','" +str(i.domain)+ "','" +str(i.type)+"','" +str(i.unit)+"','" +str(chartstr)+ "')"

				if i.mode == 'Overall':
					OveCount +=1
					querry = "INSERT INTO measurements_low(parent,id, point, raport_number, date, type, unit, value) VALUES (" + str(parent) + "," + str(i.id) + ",'"	+ str(i.point) + "','" + str(rnumber)+ "','" +str(i.date)+ "','" +str(i.type)+"','" +str(i.unit)+"','" +str(i.overall)+ "')"
				#print(i.point)
				#print(querry[:150])
				q_run(connD, querry)
				try:
		...
		def reload_lists():
			...
			window_crosstable()
		querry = 'select cs.id,nameindevice,name from crosstable as cs left join devices as dev on cs.id = dev.id where cs.parent = ' + str(parent) + ' order by name'
		crosstablequerry = q_run(connD, querry)
		querry = 'select id, point, sort from points where id IN (select id from devices where parent = '+ str(parent) +') order by id, sort'
		...
		ycord = 0
		for x in measlist:
			mylist.insert(END,str(x.routename) + ' ' + str(x.point) + ' ' + str(x.mode) + ' '+ str(x.overall) +' '+ str(x.type) + ' ' + str(x.date))
...
read_measurement_file('ezThomas')
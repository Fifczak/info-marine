	...
	class reminderwindow:
		def __init__(self):
			...
			def preQuerrys():
				...
				END as sortint, dss.sort ,dev.ignore_reminder, dev.ignore_reminder_to
			...
			def marksend():
				...
				nextfull = self.lastfulldatemin + datetime.timedelta(days=90)
					...
					for line2 in self.resultr:
						...
						if str(line[0]) == str(line2[0]): ##najnowszy na ktory trafi, potem break
							...
							if x.lastraportdate >= self.lastfulldatemin:
								...
								#print(datetoignore , datetime.datetime.now().date())
								if str(line[3]) == 'True':  # nadwarunek(devices.ignore_reminder)
									x.status = 'DISABLED'
									self.devlistbox.itemconfig(END, bg='Black')
								elif datetoignore > datetime.datetime.now().date():
...
The following equipment was recommended to be controlled in our reports:
					...
					if len(devlistnok) == 0:
						...
						devstr = chr(10) + chr(10)+ "Could you also perform measurements of machinery missing from three months survey:" + chr(
							10)
						for line in devlistnok:
							if str(line.status) == 'NOK':
								devstr += chr(10) + "-{}({})".format(line.name,line.lastraportdate)
			...
			def ColorOwners(object):
				...
				for line in self.resultrr:#tqdm(self.resultrr):
					...
					for line2 in self.reminderownerharmo:
						if line[0] == line2[0]:
							...
							if nextfull < request:
								...
								#print("RED TRUE: nextfull: {}, request_limit: {}".format(
									 #nextfull,  request))
							elif line2[2] < request:  # jeśli jest request_date mniejsza niz dzis + 2 tygodnie
								object.itemconfig(END, bg='yellow')
								#print("YELLOW TRUE: request_date: {}, request_limit: {}".format(
								   #line2[2], request))
							break
							# elif line2[1] > lastsend: #jeśli wysłano w przeciągu zeszłego tygodnia
				...
				for line in ships:
					...
					for line2 in self.reminderharmo:
						...
						if line[0] == line2[1]:
							...
							if nextfull < request:
								object.itemconfig(END, bg='red')
							if lsdate > lastsend: #jeśli wysłano w przeciągu zeszłego tygodnia
								object.itemconfig(END, bg='orange')
								break

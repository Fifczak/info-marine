...
def q_run(connD, querry):
    ...
    cur.close()
class trendchart:
    def __init__(self,ids,connD,GUI,VSG,maxdate):
            ...
            def gettrendresults():
                if VSG == False:
                    querry ="select ml.raport_number,max(ml.value),max(ml.date),dev.name from measurements_low as ml"\
                            ...
                            " where ml.id ={} and ml.type = 'RMS' and ml.value != -1" \
                                " and ml.date <= cast('{}' as date) and ml.date >= cast('{}' as date) - interval '2 years' and pts.visible = true" \
                                " and har.trendchart_visible_flag IS DISTINCT from false group by ml.raport_number,dev.name order by max(ml.date) desc".format(devid,maxdate,maxdate)
        ...
        def calldatasheet(parentclass,id_,name):
            datasheet(ids,parentclass,id_,self,name,connD,GUI,VSG,maxdate)
...
class ValRMSwindow():
    def updateRMS(self,_id_,parentclass,grandparentclass,id_,chartframe,connD,ids,GUI,maxdate):
        ...
        name = list(q_run(connD, querry))[0][0]
        datasheet(ids,chartframe,id_,chartframe,name,connD,GUI,self.VSG,maxdate)
    def __init__(self, connD, parentclass, _id_,grandparentclass,id_,chartframe,ids,GUI,VSG,maxdate):
        self.VSG = VSG
        ...
        self.okbut = tk.Button(self.window, text='UPDATE', command=lambda:self.updateRMS(_id_,self,grandparentclass,id_,chartframe,connD,ids,GUI,maxdate), width=12)
...
class measBut:
    def changeWindow(self,_id_):
        pass
    def __init__(self,dsFrame,r,c,val,_id_,parentclass,id_,chartframe,connD,ids,GUI,VSG,maxdate):
        self._id = _id_
        self.lab = tk.Button(dsFrame,text = str(val),command = lambda : ValRMSwindow(connD,self,_id_,parentclass,id_,chartframe,ids,GUI,VSG,maxdate), width = 15)
...
class datasheet:
    def quit(self):
        self.DSW.destroy()
    def refresh (self,ids,parentclass,connD,GUI,maxdate):
        parentclass.wdw.destroy()
        self.quit()
        trendchart(ids,connD,GUI,self.VSG,maxdate)
        # try:chartwindow.mainloop()
        # except: print('No chartwindow')
    def __init__(self,ids,parentclass,id_,chartframe,name,connD,GUI,VSG,maxdate):
        self.VSG = VSG
        self.DSW = tk.Tk()
        ...
        if VSG == False:
            querry = """select ml.raport_number,ml.point, ml.value,ml.date,dev.name, ml._id_ from measurements_low as ml
                        ...
                        left join harmonogram har on ml.raport_number = har.report_number
                        where ml.id = {} and ml.type = 'RMS' and ml.value != -1 and pt.visible = True 
                        and  ml.date <= cast('{}' as date) and ml.date >= cast('{}' as date) - interval '2 years' and har.trendchart_visible_flag IS DISTINCT from false
                        group by ml.raport_number,ml.point, dev.name,ml.value,ml.date ,pt.sort , ml._id_
                        order by ml.date ,pt.sort  """.format(str(id_),maxdate,maxdate)
        else:
            ...
            for line2 in PointList:
                ...
                for seekVal in dsd:
                    if str(line) == str(seekVal[0]) and str(line2) == str(seekVal[1]):
                        x = measBut(dsFrame,r,c,seekVal[2],seekVal[5],self,id_,chartframe,connD,ids,GUI,self.VSG,maxdate)
        dsFrame.pack()
        okbutton = tk.Button(self.DSW,text = 'Reload Chart', command = lambda:self.refresh(ids,parentclass,connD,GUI,maxdate)).pack()
        self.DSW.mainloop()

#trendchart(['14692','14693'], ['testuser','info','192.168.10.243'],True,False,'2019-08-20')

...
def resulttable_iloar(tn, host, username, password, tableno, id1, id2, id3, id4, id5, id6, rn):
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id1) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id1) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id2) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id2) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id1) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id1) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id2) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id2) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id3) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id3) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id4) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id4) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id3) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id3) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id4) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id4) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id5) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id5) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'RMS' and ml.id = " + str(id6) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id6) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	result = []
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id5) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id5) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where type = 'envelope P-K' and ml.id = " + str(id6) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id6) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
...
def makereport(username, password, host, rn_, id_): #AUTORUN!
	...
	for j in range (4):
		...
		resulttable_iloar(j, host, username, password, table6, motors[(((j)*3))], thrusters[(((j)*3))], motors[(((j)*3)+1)], thrusters[(((j)*3)+1)], motors[(((j)*3)+2)], thrusters[(((j)*3)+2)], rn_)
		...
		r=chart.add_run(' \n Fig.'+ str((j*12)+2) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(SB turn) el. motor')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+3) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(SB turn) el. motor')
		#r=chart.add_run(str(motors[(((j)*3))]))
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+4) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(SB turn) gear')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+5) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(SB turn) gear')
		#r=chart.add_run(str(thrusters[(((j)*3))]))
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+6) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(PS turn) el. motor')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+7) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(PS turn) el. motor')
		#r=chart.add_run(str(motors[(((j)*3)+1)]))
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+8) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(PS turn) gear')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+9) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(PS turn) gear')
		#r=chart.add_run(str(thrusters[(((j)*3)+1)]))
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+10) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(idle) el. motor')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+11) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(idle) el. motor')
		#r=chart.add_run(str(motors[(((j)*3)+2)]))
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+12) +' FFT velocity graph point ' + str(maxPoint) + ' on ' + thname + '(idle) gear')
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*12)+13) +' FFT envelope graph point ' + str(maxPoint) + ' on ' + thname + '(idle) gear')
	...
	if id_ == '53':
		...
		document.save('C:\overmind\Reports\Diagnostic report Iolar Platform ' + str(rn_) +'_' + str(ini[0][0]) + '_' + str(repdate) + '.docx')
	elif id_ == '55':
		document.save('C:\overmind\Reports\Diagnostic report Neptuno Platform ' + str(rn_) +'_' + str(ini[0][0]) + '_' + str(repdate) + '.docx')

...
def resulttable(tn, host, username, password, tableno, id1, id2, rn):
	...
	cur = conn.cursor()
	querry = "select value from (select distinct ml.value, ml.point, pts.sort from measurements_low as ml RIGHT JOIN points as pts ON ml.id = pts.id and ml.point = pts.point where ml.id = " + str(id2) + " and ml.raport_number = '" + str(rn) + "' and ml.point IN (select point from points as pts where id = " + str(id2) + " order by sort)) as al order by al.sort"
	cur.execute(querry)
	result = cur.fetchall()
	conn.commit()
	cur.close()
...
def makereport(username, password, host, rn_, id_): #AUTORUN!
	...
	for j in range (6):
		...
		r=chart.add_run(' \n Fig.'+ str((j*2)+2) +' FFT velocity graph point ' + str(maxPoint) + ' on Thruster ' + str(j+1) + ' el. Motor')
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		...
		r.add_picture('C:\\overmind\\temp\\tempchart.png', width=Inches(7.0))
		os.remove('C:\\overmind\\temp\\tempchart.png')
		r=chart.add_run(' \n Fig.'+ str((j*2)+3) +' FFT velocity graph point ' + str(maxPoint) + ' on Thruster ' + str(j+1) + ' gear')
		r.font.name = 'Calibri'
		r.font.size = Pt(9)
		r.font.italic = True
		chart.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER
